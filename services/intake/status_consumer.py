...
def process_message(db, message: dict) -> None:
    ...
    if payment is None:
        # Nothing to update -- still delete so this message doesn't
        # get stuck retrying forever against a payment that doesn't exist.
        sqs.delete_message(QueueUrl=get_queue_url(), ReceiptHandle=message["ReceiptHandle"])
        return

    payment.status = payload["decision"]  # "approved" or "review"
    db.commit()
